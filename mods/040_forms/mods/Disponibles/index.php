...
<html>
	...
	<body>
		<div data-role='page' id='forms' data-theme='b'>
...
if($forms_no->num_rows){
	...
	foreach($forms_no_array as $form){
		...
		$form_id = $form[0]['id'];
		$form_id2 = $form[0]['form_id'];
		$form_limit = $form[0]['limit_date'];
		echo "<li>
				<div data-role='collapsible' id='fui_$form_id' data-collapsed='true' data-theme='b' data-content-theme='d'>
					<h3><div style='float:left;'>$form_name</div><div style='float:right;'>Fecha limite: $form_limit</div></h3>
					<form method='post' action='send.php' enctype='multipart/form-data' method='post' data-ajax='false'>
						<ul style='list-style:none; padding:0;'>";
		$file = glob("../../template/fid_$form_id2.*");
		if(count($file)>0){
			$file = str_replace('../../', '', $file[0]);
			echo "<li style='margin-top:10px;'><a rel='external' href='../../download.php?f=$file'>Fichero Adjunto!</a></li>";
		}
					
		if($form_limit > date('Y-m-d H:i:s')){
			echo "<li><input type='hidden' name='form_id' value='$form_id'/></li>";
			foreach($form as $element){
				...
				switch ($element_type) {
					case 1:
						echo "<li><input type='text' name='$element_id' placeholder='$element_name'/></li>";
						break;
					case 2:
						echo "<li><input type='number' name='$element_id' placeholder='$element_name'/></li>";
						break;
					case 3:
						echo "<li><input type='password' name='$element_id' placeholder='$element_name'/></li>";
						break;
					case 4:
						echo "<li><label>$element_name</label><input type='datetime-local' name='$element_id' placeholder='$element_name'/></li>";
						break;
					case 5:
						echo "<li><label>$element_name</label><input type='file' name='$element_id' placeholder='$element_name'/></li>";
						break;
					case 6:
						echo "<li><textarea name='$element_id' placeholder='$element_name'></textarea></li>";
						break;
				}
			}
			echo "<li><input type='submit' value='Enviar'/></li>";
		}else
			echo '<li>Tiempo de entrega exedido!</li>';
		echo "
						</ul>
					</form>
				</div>
