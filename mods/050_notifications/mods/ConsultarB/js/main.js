...
clientList.put = function(clientsString){
	...
	for(var i = 0; i < clients.length - 1; i++){
		...
		var clientSect = "" +
			"<li data-theme='c' id='"+clientInfo[0]+"' class='headMsg'><a href='notifi.php?c="+clientInfo[1]+"'>" +
			"<span class='ui-li-count'>"+clientInfo[0]+"</span>" +
			"	<h3>" + clientInfo[2] + "	<font size='2' color='#FF7373'>" + clientInfo[1] + "</font>" +
...
$(document).bind('pageshow', function() {
	...
	if(currentPage == 'documents'){
		...
		$('.headMsg').bind('expand', function(e){
			if(!$(this).hasClass('showed')){
				var obje = $('#'+this.id+' .ui-collapsible-heading-toggle')[0];
				$(obje).attr('data-theme', 'b').removeClass("ui-btn-up-e").addClass("ui-btn-up-b").removeClass("ui-btn-hover-e").addClass("ui-btn-hover-b").trigger('create');
				setShowed(this.id);
				$(this).addClass('showed');
			}			
		}).bind('collapse', function(e){
			//$('#'+this.id+' .ui-collapsible-heading-toggle')[0].removeClass("ui-btn-up-e").addClass("ui-btn-up-b");
		});
	}
