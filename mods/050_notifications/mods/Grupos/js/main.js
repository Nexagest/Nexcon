...
clientList.put = function(clientsString){
	...
	for(var i = 0; i < clients.length - 1; i++){
		...
		var clientSect = "" +
			"<li data-theme='b' id='" + clientInfo[0] + "' tname='"+ clientInfo[1] +"' ttext='"+ clientInfo[2] +"'><a href='#' onclick='typeoptions(\"" + clientInfo[0] + "\");'>" +
			"	<h3>" + clientInfo[1] + "	<font size='2' color='#FF7373'>( " + clientInfo[2] + " )</font></h3>" +
			"</a></li>" +
		"";
	...
	if(currentPage == 'documents'){
		$('#documents').ready(function(){
			...
			$('#addType').bind('vclick', function(){
				nx.goPage('add.php?t='+$('#newTypeText').val()+'&n='+$('#newTypeName').val());
			});
			$('#mod').bind('vclick', function(){
				nx.goPage('add.php?i='+currentType+'&n='+$('#nameMod').val()+'&t='+$('#textMod').val());
			});
